...
Closes most of `docs/specs/platform-cross-cutting.md`'s priority-ordered work items — role-based authorization gaps, a ledger concurrency correctness bug, and one of the four scoring-integrity residuals v1.5 explicitly left open. Account recovery (the fourth priority item) was a deliberate product decision, not an implementation gap; see below.

- **Ledger and chama endpoints are now actually role-scoped, not just nav-hidden.** `get_current_user` only ever validated the JWT, never the role, on `/ledger/*` and `/chama/*` — a logged-in `UNDERWRITER` or `ADMIN` account could call `POST /ledger/transactions/sale` directly; the frontend simply never showed them the option, which was never a security boundary. `/ledger/*` is now `require_role("MERCHANT")` only (a `CHAMA_MEMBER` has no shop; letting them write to the ledger was a correctness gap, not just a hardening gap). `/chama/*` is now `require_role("MERCHANT", "CHAMA_MEMBER")`, per §1.4's "merchants participate in chamas too."
- **`POST /chama/payouts` gained an officer-only check it never had.** Discovered while touching the surrounding code for the role-scoping fix above: *any* authenticated user (not just chama members) could schedule a payout for *any* chama. Now requires `CHAIRPERSON`/`TREASURER`/`SECRETARY` membership in that specific chama, mirroring the duty-separation check `RecordContribution` already established in v1.5.
- **Fixed the `sequence_no` race flagged as "tracked for implementation" back in v1.3.** `_AppendLedgerEntry.execute` now takes a Postgres advisory transaction lock (`pg_advisory_xact_lock(hashtext(merchant_id))`) before reading the merchant's last transaction, serializing concurrent appends for the *same* merchant without affecting throughput for different merchants. Previously, two simultaneous sale/expense/supplier-payment requests for one merchant could both read the same "last" row and race on the `UNIQUE(merchant_id, sequence_no)` constraint — the constraint prevented a corrupted chain, but the loser just failed outright with no retry. Verified with a test that reproduces the race (reliably fails without the lock, reliably passes with it) plus a separate test confirming two different merchants are never serialized against each other.
- **Closes one of the four scoring-integrity items v1.5 left open: "no statistical anomaly/outlier detection exists to flag suspicious patterns... the score is presented as clean and final with no fraud-risk signal alongside it."** `EvaluateMerchant` now computes two cheap heuristic flags alongside every score — `VOLUME_SPIKE_LAST_3_DAYS` (more than 80% of trailing-30-day revenue occurred in the last 3 days) and `IMPLAUSIBLY_SMOOTH_MARGIN` (near-zero variance in day-level margin ratio across at least 3 distinct sale days) — stored in a new `credit_scores.anomaly_flags` JSONB column (a sibling to `shap_explanation`, not folded into it, since that dict stays a pure feature→SHAP-value mapping) and surfaced to underwriters as a warning badge in `ApplicantTable.tsx`. **This is explicitly a triage heuristic, not a fraud model** — it will false-positive on, e.g., a legitimate single-SKU business (one product means margin variance is always exactly zero). Deliberately excluded from the merchant's own self-view endpoint (`CreditScoreOut`/`GET /scoring/scores/{user_id}/latest`) — only `ApplicantOut` (the underwriter-facing `GET /underwriter/applicants` schema) includes it, so a merchant can't see exactly which heuristic tripped and learn how to evade it. The other three scoring-integrity items from v1.5's "still open" list (payment-rail corroboration, evaluation-timing gaming, the `receivables_days` proxy) remain open — each needs its own design pass, not a quick follow-on to this change.
- **Account recovery (losing your registered phone number) is a resolved product decision, not an implementation gap.** Considered: (a) a secondary contact method + manual verification, (b) full KYC-document-based recovery, (c) support-mediated deactivation + fresh registration. Decided on (c): a user who loses their phone number contacts support, an admin deactivates the old account via the existing `POST /admin/users/{id}/deactivate`, and the user re-registers under a different number through the ordinary OTP flow. This uses only existing capabilities — no new schema, endpoint, or UI. Explicitly accepted tradeoff: this does not preserve continuity (credit history, chama membership) from the old account, and `users.phone_number`'s uniqueness means the *old* number specifically can't be reclaimed by the new account since the deactivated row still occupies it. If that turns out to matter in practice, revisit with option (a) or (b) above rather than retrofitting number-reclamation onto this path.
